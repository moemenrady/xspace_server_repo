    ...
    <div class="card">
        ...
        <div class="section client-main">
            <div class="box client-info">
                <div class="row">
                    <div class="col">
                        <label class="checkbox"><input type="checkbox" checked disabled> <span class="lbl">اسم</span></label>
                        <p class="value">{{ $client->name }}</p>
                    </div>
                    <div class="col">
                        <label class="checkbox"><input type="checkbox" checked disabled> <span class="lbl">رقم الهاتف</span></label>
            ...
            <div class="box flex-grid">
                ...
                <div class="status-card">
                    ...
                    @if($subscription)
                        ...
                        <div class="actions">
                            <a href="{{ route('subscriptions.show', $subscription->id) }}" class="btn small">🔍 تفاصيل الاشتراك</a>
                        </div>
                    @else
                ...
                <div class="status-card">
                    ...
                    @if($bookings->count())
                        ...
                        <div class="actions">
                            <a href="{{ route('client.bookings', $client->id) }}" class="btn small">🔍 تفاصيل الحجوزات</a>
                        </div>
                    @else
                ...
                <div class="status-card">
                    ...
                    @if($activeSession)
                        <p class="small">تبدأ: {{ \Carbon\Carbon::parse($activeSession->start_time)->format('Y-m-d H:i') }}</p>
                        <div class="actions">
                            <a href="{{ route('session.show', $activeSession->id) }}" class="btn small">🔍 فتح الجلسة</a>
                        </div>
                    @else
                        ...
                        @if($bookings->count())
                            <ul class="mini-list">
                                @foreach($bookings->take(3) as $b)
                                    <li>
                                        {{ $b->title }} — {{ $b->status }} — {{ \Carbon\Carbon::parse($b->start_at)->format('Y-m-d H:i') }}
                                    </li>
                                @endforeach
                            </ul>
                        @else
                        ...
                        @if($recentSessions->count())
                            <ul class="mini-list">
                                @foreach($recentSessions->take(3) as $s)
                                    <li>بدء: {{ \Carbon\Carbon::parse($s->start_time)->format('Y-m-d H:i') }} — حالة: {{ $s->status }}</li>
                                @endforeach
                            </ul>
                        @else
...
<style>
    ...
    /* layout */
    .flex-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 14px; }
    .status-card { background: #fff; padding: 12px; border-radius: 12px; box-shadow: 0 2px 12px rgba(0,0,0,0.04); display:flex; flex-direction:column; gap:8px; min-height:110px; }
    .d-check { display:flex; align-items:center; gap:8px; font-weight:600; }
    .d-check input { transform: scale(1.1); margin-right:6px; }
    .actions { margin-top:6px; }
    .btn.small { display:inline-block; background:#D9B1AB; color:#fff; padding:8px 12px; border-radius:10px; text-decoration:none; font-weight:700; box-shadow: 0 4px 10px rgba(0,0,0,0.06); transition: transform .18s; }
    .btn.small:hover { transform: translateY(-3px); background:#a86f68; }

    .muted { color:#7a7a7a; }
    .value { font-size:16px; font-weight:700; margin-top:6px; color:#222; }
    .row { display:flex; gap:20px; }
    .col { flex:1; }

    .quick-grid { display:flex; gap:18px; }
    .mini-list { list-style:none; padding-left:0; margin:0; font-size:14px; color:#333; }
