...
@section('content')
    ...
    <div class="sale-container">
        ...
        <!-- زرار عرض الفاتورة -->
        <form id="invoiceForm" action="{{ route('booking.purchase.store', $booking->id ?? 0) }}" method="POST"
            class="mt-3">
            @csrf
            <input type="hidden" name="items" id="itemsInput">
            <button type="submit" class="invoice-btn">أضافة للحجز</button>
        </form>
        ...
        (function($) {
            ...
            function renderResults(data) {
                ...
                if (!data || data.length === 0) {
                    $(sel.results).html('<div class="list-group-item text-muted">لا توجد نتائج</div>').removeClass(
                        'd-none');
                    currentResults = [];
                    return;
                }
      ...
      `);
            ...
            function removeItemAt(idx) {
                ...
                if (itemsList.length === 0) {
                    ...
                    itemsFocusedIndex = itemsList.length - 1;
                }
                renderItems();
            }
            ...
            function addSelectedProduct() {
                if (!selectedProduct) {
                    alert("⚠️ اختر منتج من البحث الأول");
                    return false;
                }
                if (!qtyInput || qtyInput <= 0) {
                    alert("⚠️ اختر عدد صحيح");
                    return false;
                }
            ...
            function doSearch(query) {
                ...
                searchDebounceTimer = setTimeout(() => {
                    $.ajax({
                        ...
                        error: function() {
                            $(sel.results).html(
                                    '<div class="list-group-item text-muted">خطأ في البحث</div>')
                                .removeClass('d-none');
                            currentResults = [];
                            highlightedIndex = -1;
                        }
            ...
            function trySubmitInvoiceForm() {
                const $form = $(sel.invoiceForm);
                const $hidden = $(sel.itemsInputHidden);
                if ($form.length && $hidden.length) {
                    $hidden.val(JSON.stringify(itemsList));
                    $form.trigger('submit');
                    return true;
                }
                return false;
